...
export class AgentSessionWrapper {
  ...
  private notifyAgentRunCompleteIfIdle(): void {
    ...
    try {
      ...
      console.error(
        "[pi-web-x] completion listener failed:",
  ...
  beginExtensionBinding(): void {
    void this.ensureExtensionsBound().catch((err) => {
      console.error(
        "[pi-web-x] failed to dispatch session_start to extensions:",
  ...
  private ensureExtensionsBound(): Promise<void> {
    ...
    this.extensionBindingPromise = (async () => {
      ...
      this.applyExactSystemPrompt();
      console.log(
        `[pi-web-x] session_start dispatched to extensions for session ${this.inner.sessionId}`,
      );
    })().catch((err) => {
  ...
  private emit(event: AgentEvent): void {
    for (const listener of this.listeners) {
      try {
        ...
        console.error(
          `[pi-web-x] failed to deliver ${event.type} event:`,
          error instanceof Error ? error.message : error,
        );
      }
    }
  ...
  private async shutdownAfterSessionReplacement(
    ...
    try {
      ...
      console.error(
        `[pi-web-x] ${replacement} succeeded, but source session shutdown failed:`,
        error instanceof Error ? error.message : error,
      );
    }
  }
        ...
        case "prompt": {
          ...
          try {
            ...
            void prompt
              ...
              .catch((error) => {
                console.error(
                  "[pi-web-x] prompt completion handler failed:",
                  error instanceof Error ? error.message : error,
                );
              });
  ...
  destroy(): void {
    ...
    void (async () =>
      ...
      .catch((error) => {
        console.error(
          "[pi-web-x] session_shutdown before dispose failed:",
          error instanceof Error ? error.message : error,
        );
      })
    ...
    this.shutdownPromise = (async () => {
      try {
        try {
          ...
          console.error(
            "[pi-web-x] extension binding failed before session shutdown:",
            error instanceof Error ? error.message : error,
          );
        }
        if (!this.sessionShutdownEmitted) {
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      setTheme: () => ({
        success: false,
        error: "Theme switching is not supported in Pi Web X extension UI yet",
      }),
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    if (!subagentResources && selectedToolNames !== undefined) {
      ...
      // tool registry — so they were unavailable in Pi Web X sessions even though the
    ...
    // extensions stay usable in Pi Web X just like in the `pi` CLI.
    ...
    const wrapper = new AgentSessionWrapper(inner, {
      ...
      onAgentRunComplete: (completedSessionId) => {
        void notifySessionComplete(completedSessionId).catch((error) => {
          console.error(
            "[pi-web-x] failed to send completion push:",
            error instanceof Error ? error.message : error,
          );
        });
      },
