...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "auth.setupDescription": "这是 Pi Web X 首次运行。请输入服务器启动日志中打印的设置令牌，并设置访问密码以启用访问认证。",
    "auth.loginTitle": "登录",
    "auth.password": "密码",
    ...
    "auth.changePasswordDescription": "修改密码会使所有设备退出登录，之后需要重新登录。",
    "auth.logout": "退出登录",
    "auth.logoutDescription": "结束本设备上的当前会话。",
    "auth.submit": "继续",
    "auth.processing": "处理中…",
    "auth.saveAndRelogin": "保存并使所有设备退出登录",
    "auth.setupHint": "设置令牌仅在服务器启动日志中出现一次，请保持本页面打开以便复制。",
    ...
    "auth.error.AUTH_PASSWORD_INVALID": "密码长度需为 8-128 字符，且不能是常见弱密码。",
    ...
    "pwa.insecureConnection": "当前 HTTP 连接无法提供安装、后台通知或可靠的离线支持。",
    "pwa.authenticationUnavailable": "此连接尚未启用 Web Access Authentication。",
    "pwa.connectionSafetyAdvice": "在本设备以外访问 Pi Web X 时，请使用 HTTPS、可信 VPN 和 Web Access Authentication。",
    ...
    "pwa.notificationDescription": "开启通知后，后台任务结束时将提醒你。通知只显示会话名称和状态。",
    "pwa.notNow": "暂不",
