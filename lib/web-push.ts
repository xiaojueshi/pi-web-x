  ...
  return {
    async send(subscription, payload, vapidKeys) {
      await webpush.sendNotification(
        ...
        {
          vapidDetails: {
            subject: "mailto:pi-web-x@localhost",
...
export function localeText(
  locale: string,
  key: "sessionComplete" | "taskFinished",
): string {
  if (locale === "zh-CN") {
    const message =
      zhCNLocale.messages[
        key === "sessionComplete" ? "i18n.sessionComplete" : "i18n.taskFinished"
      ];
  ...
  return (
    message ??
    (key === "sessionComplete" ? "Session complete" : "Task finished.")
  );
}
