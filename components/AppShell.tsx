...
import { useRouter, useSearchParams } from "@/src/client/navigation";
...
export function AppShell() {
  ...
  const handleSystemInfoToggle = useCallback(
    (panel: "system" | "tools", keepMobileToolbarOpen = false) => {
  ...
  const handleFileViewerStateChange = useCallback(
    (tabId: string, viewerRevision: number, viewerState: FileViewerState) => {
      setFileTabs((prev) =>
        saveFileViewerState(prev, tabId, viewerRevision, viewerState),
      );
    },
    [],
  );
      ...
      void fetch("/api/sessions")
        ...
        .then((d) => {
          ...
          if (
            ...
            router.replace(`?session=${encodeURIComponent(s.id)}`, {
              scroll: false,
            });
          }
        })
  ...
  const handleOpenSession = useCallback(
    async (sessionId: string) => {
      try {
        ...
        console.error(
          "[pi-web-x] failed to open session:",
    ...
    ({
      ...
      const fire = () => {
        const sessionUrl = targetSession
          ? `/?session=${encodeURIComponent(targetSession.id)}`
          : "/";
  ...
  const handleAgentEnd = useCallback(() => {
    ...
    deliverSessionNotification({
      ...
      tag: targetSession
        ? `pi-session-complete:${targetSession.id}`
        : "pi-session-complete",
    });
  }, [
    deliverSessionNotification,
  ...
  const handleAttentionNeeded = useCallback(
    (request: BlockingExtensionUiRequest) => {
      ...
      if (
        !claimExtensionAttentionNotification(
          request,
          notifiedAttentionRequestIdsRef.current,
        )
      )
        return;
      ...
      if (selectedSession?.id === sessionId) {
        ...
        const draftId =
          typeof crypto.randomUUID === "function"
            ...
            : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
        setNewSessionDraftId(draftId);
        activeNewSessionDraftKeyRef.current = cwd
          ? `new:${draftId}:${cwd}`
          : null;
        setSelectedSession(null);
  ...
  const newSessionDraftKey =
    selectedSession === null && effectiveNewSessionCwd
  ...
  const renderProjectTrustWarning = (mobileBanner: boolean) => {
    if (!showChat || !projectTrust?.requiresTrust || projectTrust.trusted)
      return null;
    return (
      <button
        type="button"
        ...
        style={{
          ...
          background: mobileBanner
            ? "color-mix(in srgb, #d97706 8%, var(--bg-panel))"
            : "none",
          border: "none",
      ...
      <div style={{ display: "flex", alignItems: "stretch", height: "100%" }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                opacity:
                  disabled && autoNameStatus.kind !== "naming" ? 0.45 : 1,
                flexShrink: 0,
                fontSize: 11,
                whiteSpace: "nowrap",
                transition: "color 0.1s, background 0.1s, opacity 0.1s",
              ...
              onMouseEnter={(event) => {
                if (disabled) return;
                event.currentTarget.style.color = isError
                  ? "#dc2626"
                  : "var(--text)";
                event.currentTarget.style.background = "var(--bg-hover)";
              }}
        ...
        {hasSubagentSessions && (
          <button
            ...
            style={{
              ...
              padding: mobile ? 0 : "0 12px",
              background:
                activeTopPanel === "agents" ? "var(--bg-selected)" : "none",
              border: "none",
              borderTop:
        ...
        <button
          ...
          style={{
            ...
            padding: mobile ? 0 : "0 12px",
            background:
              activeTopPanel === "tools" ? "var(--bg-selected)" : "none",
            border: "none",
            borderTop:
            ...
            cursor: mobile && !showChat ? "not-allowed" : "pointer",
            color:
              activeTopPanel === "tools" ? "var(--text)" : "var(--text-muted)",
            opacity: mobile && !showChat ? 0.45 : 1,
            fontSize: 11,
            whiteSpace: "nowrap",
            transition: "color 0.1s, background 0.1s",
        ...
        >
          <svg
            ...
            strokeLinejoin="round"
            style={{
              color: systemTools?.some((tool) => tool.active)
                ? "var(--accent)"
                : "var(--text-dim)",
              flexShrink: 0,
            }}
  ...
  const renderSessionStatsButton = (mobile: boolean) => {
    ...
    const formatCompact = (value: number) =>
      value >= 1_000_000
        ...
        : value >= 1000
          ...
          : String(value);
    const costText =
      cost > 0 ? (cost >= 0.01 ? `$${cost.toFixed(2)}` : `<$0.01`) : null;
    ...
    if (contextUsage?.contextWindow) {
      ...
      if (percent !== null && percent > 90) contextColor = "#ef4444";
        ...
        contextColor = "rgba(234,179,8,0.95)";
      desktopContextText =
        percent !== null
        ...
        {mobile ? (
          <>
            {tokens && tokens.input > 0 && (
              <span
                className="mobile-session-stat-io"
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 2,
            ...
            {costText && (
              <span
                className="mobile-session-stat-cost"
                style={{ color: "var(--text)", fontWeight: 500, flexShrink: 0 }}
              >
                {costText}
              </span>
            )}
  ...
  const renderMainFileToggle = (mobile: boolean) => {
    ...
    return (
      <button
        ...
        onMouseEnter={(event) => {
          if (!covered) event.currentTarget.style.color = "var(--text)";
        }}
        onMouseLeave={(event) => {
