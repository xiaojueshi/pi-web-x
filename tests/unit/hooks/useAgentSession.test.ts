...
test("keeps live following cancellable when the user scrolls away from the tail", () => {
  ...
  assert.match(
    ...
    /messagesEndRef\.current\?\.scrollIntoView\(\{ behavior \}\);\s*if \(container\) previousScrollTopRef\.current = container\.scrollTop/,
  );
...
test("uses the prompt anchor as the only trailing message spacer", () => {
  assert.match(
    chatWindowSource,
    /<div ref=\{promptAnchorSpacerRef\} aria-hidden="true" \/>[\s\S]*?<div ref=\{messagesEndRef\} \/>/,
  );
  assert.doesNotMatch(
    chatWindowSource,
