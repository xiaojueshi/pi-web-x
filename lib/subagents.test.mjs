...
const { isSubagentProfileOverridden } = await createJiti(
  import.meta.url,
).import("./subagent-profile-precedence.ts");
...
test("an empty tool selection round-trips without restoring default tools", async () => {
  ...
  try {
    ...
    const loaded = listSubagentProfiles(cwd).find(
      (item) => item.name === saved.name,
    );
    const source = await readFile(
      join(cwd, ".pi", "agents", `${saved.name}.md`),
      "utf8",
    );
  ...
  try {
    ...
    assert.throws(
      () =>
        saveProjectSubagentProfile(
          cwd,
          profile({ maxTurns: Number.POSITIVE_INFINITY }),
        ),
...
test("project profiles override workspace profiles and deletion restores the workspace version", async () => {
  ...
  try {
    ...
    saveProjectSubagentProfile(
      cwd,
      profile({ description: "Project version" }),
    );
    assert.equal(
      resolveSubagentProfile(cwd, "TEST-AGENT").description,
      "Project version",
    );
...
test("global and project sources with the same name stay visible while project wins at runtime", async () => {
  ...
  try {
    saveSubagentProfile(
      cwd,
      "global",
      profile({ description: "Global version" }),
    );
    saveSubagentProfile(
      cwd,
      "project",
...
test("global profiles round-trip and deleting an override restores the built-in", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-x-subagents-"));
  try {
    const saved = saveSubagentProfile(
      cwd,
      "global",
      profile({
        name: "Explore",
    ...
    assert.equal(
      resolveSubagentProfile(cwd, "Explore").description,
      "Global override",
    );
...
test("disabled profiles cannot be resolved for execution", async () => {
  ...
  try {
    ...
    const sources = listSubagentProfileSources(cwd).filter(
      (item) => item.name === "test-agent",
    );
    ...
    assert.equal(
      listSubagentProfiles(cwd).find((item) => item.name === "test-agent")
        .enabled,
      false,
    );
  } finally {
...
test("persisted runs distinguish interrupted, failed, aborted, and latest results", () => {
  ...
  assert.equal(
    readSubagentRun([meta], "child", "/tmp/child.jsonl").status,
    "interrupted",
  );
  ...
  assert.equal(
    readSubagentRun([meta, failed], "child", "/tmp/child.jsonl").status,
    "failed",
  );
  assert.equal(
    readSubagentRun([meta, failed], "child", "/tmp/child.jsonl").error,
    "boom",
  );
  assert.equal(
    readSubagentRun([meta, failed, aborted], "child", "/tmp/child.jsonl")
      .status,
    "aborted",
  );
  assert.equal(
...
test("project profile directories cannot escape cwd through symbolic links", async (t) => {
  ...
  assert.equal(
    listSubagentProfileSources(cwd).some((item) => item.name === "secret"),
    false,
  );
  assert.equal(
    listSubagentProfiles(cwd).some((item) => item.name === "secret"),
    false,
  );
  assert.throws(
