...
export async function POST(req: Request) {
  if (!isApiRequestAllowed(req)) {
    return HttpResponse.json(
      { ok: false, error: "Untrusted API request" },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    const modelId =
      typeof body.model.id === "string" ? body.model.id.trim() : "";
    if (!modelId)
      return HttpResponse.json(
        { ok: false, error: "Model ID is required" },
        { status: 400 },
      );

    tempDir = mkdtempSync(join(tmpdir(), "pi-web-x-model-test-"));
    ...
    if (loadError) return HttpResponse.json({ ok: false, error: loadError });
