  ...
  const cases = [
    {
      options: {
        agentBinDir,
        baseEnvironment: { Path: "project-metadata", PATH: "/usr/bin" },
        platform: "linux",
      },
      expected: {
...
test("a user extension keeps priority over the Pi Web X fallback bash tool", async () => {
  const userBash = {
    name: "user-bash",
    factory: (pi) => {
      pi.registerTool({
        ...
        async execute() {
          return {
            content: [{ type: "text", text: "user override" }],
            details: undefined,
          };
        },
      });
