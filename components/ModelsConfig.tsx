...
function readRememberedSelection(): Selection | null {
  ...
  try {
    ...
    if (
      (selection.type === "oauth" || selection.type === "apikey") &&
...
function customSelectionExists(
  config: ModelsJson,
  selection: Selection,
): boolean {
  if (selection.type === "provider")
    return Boolean(config.providers?.[selection.name]);
  if (selection.type !== "model") return true;
  return Boolean(
    config.providers?.[selection.providerName]?.models?.[selection.index],
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Field label="Headers">
        ...
        />
        <span style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>
          Added to every request from this provider (e.g. User-Agent). Useful
          for gateways with bot detection.
        </span>
      </Field>
...
function HeaderListEditor({
  ...
  const [rows, setRows] = useState<HeaderRow[]>(() =>
    Object.entries(headers ?? {}).map(([name, value], id) => ({
      id,
      name,
      value,
    })),
      ...
      {rows.map((row) => (
        <div key={row.id} style={{ display: "flex", gap: 6 }}>
          ...
          <input
            ...
            style={{ ...inputStyle, fontFamily: "var(--font-mono)", flex: 1 }}
          />
          <button onClick={() => removeEntry(row.id)} style={rowBtnStyle}>
            ✕
          </button>
        </div>
      ))}
...
function ModelDetail({
  ...
  const [costDraft, setCostDraft] = useState<ModelCostDraft>(() =>
    modelCostToDraft(model.cost),
  );
  ...
  const advancedSummaryParts = [
    ...
    Object.keys(model.headers ?? {}).length
      ? t("models.headersSummary", {
          count: Object.keys(model.headers ?? {}).length,
        })
      : null,
    ...
    Object.keys(model.thinkingLevelMap ?? {}).length
      ? t("models.thinkingSummary", {
          count: Object.keys(model.thinkingLevelMap ?? {}).length,
        })
      : null,
        ...
        >
          <Field label={t("models.contextWindow")}>
            <NumInput
              ...
              onChange={(v) =>
                set("contextWindow", v ? parseInt(v) : undefined)
              }
              placeholder="128000"
            />
          </Field>
          <Field label={t("models.maxOutputTokens")}>
            <NumInput
              value={
                model.maxTokens !== undefined ? String(model.maxTokens) : ""
              }
        ...
        <div style={{ marginTop: 16 }}>
          ...
          {costEditing ? (
            ...
            >
              ...
              {hasModelCostDraftValue(costDraft) &&
                !parseCompleteModelCost(costDraft) && (
                  <div
                    aria-live="polite"
                    style={{
                      gridColumn: "1 / -1",
                ...
                return (
                  <div key={key} style={{ minWidth: 0 }}>
                    ...
                    <div
                      style={{
                        marginTop: 3,
                        color: missing ? "var(--text-dim)" : "var(--text)",
                        fontSize: 12,
                        fontFamily: "var(--font-mono)",
            ...
            {model.reasoning && (
              ...
              >
                ...
                <Check
                  ...
                  checked={
                    effectiveCompat(provider, model)[
                      "supportsDeveloperRole"
                    ] !== false
                  }
                  onChange={(v) =>
                    onChange(setCompatBool(model, "supportsDeveloperRole", v))
                  }
                />
...
export function ModelsConfig({
    ...
    fetch("/api/auth/providers")
      ...
      .then(
        (d: {
          ...
          if (Array.isArray(d.oauthProviders))
            setOauthProviders(d.oauthProviders);
          if (Array.isArray(d.apiKeyProviders))
            setApiKeyProviders(d.apiKeyProviders);
        },
      )
      .catch(() => {});
  }, []);
    ...
    fetch("/api/models-config")
      ...
      .then((d: ModelsJson) => {
        ...
        setSelection((current) =>
          current && customSelectionExists(normalized, current)
            ? current
            : keys[0]
  ...
  useEffect(() => {
    if (selection)
      setLastSettingsSelection("models", JSON.stringify(selection));
  }, [selection]);
