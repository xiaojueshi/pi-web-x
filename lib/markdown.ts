...
export function normalizeDisplayMath(markdown: string): string {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    const looseBracketDisplayOneLine = line.match(
      /^([ ]{0,3})\[[ \t]*(.+?)[ \t]*\][ \t]*$/,
    );
...
function findDisplayMathClose(
  ...
  if (knownUnmatchedUntil !== undefined && startIndex < knownUnmatchedUntil)
    return null;

  for (let index = startIndex; index < lines.length; index++) {
    const line = lines[index];
    if (isDisplayMathFence(line, indent))
      return { index, content: "", glued: false };
...
function isLikelyMathExpression(value: string): boolean {
  return (
    /\\[A-Za-z]+/.test(value) &&
    !/\b(?:https?|file|mailto):|\b[A-Za-z]:\\|^\\\\/i.test(value)
  );
}
...
export const markdownPreviewRemarkPlugins: ReactMarkdownOptions["remarkPlugins"] =
  [[remarkFrontmatter, ["yaml"]], [remarkGfm, remarkGfmOptions], remarkMath];
