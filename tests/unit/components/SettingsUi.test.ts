...
const templateSource = await readFile(
  new URL("../../../components/SettingsUi.tsx", import.meta.url),
  "utf8",
);
...
test("provides one template for config layout and controls", () => {
  ...
  assert.match(
    cssSource,
    /@media \(max-width: 640px\)[\s\S]*?\.config-sidebar \{[\s\S]*?width: 100%/,
  );
  assert.match(
    cssSource,
    /@media \(max-width: 640px\)[\s\S]*?\.config-detail \{[\s\S]*?padding: 14px/,
  );
});
...
test("all subpanel sidebars share one typography scale", () => {
  ...
  assert.match(
    cssSource,
    /\.config-sidebar-text \{[\s\S]*?font-family: inherit[\s\S]*?font-size: 12px/,
  );
  assert.match(
    cssSource,
    /\.config-sidebar-group-label \{[\s\S]*?font-family: inherit[\s\S]*?font-size: 10px/,
  );
...
test("skills and sub-agents share interactive sidebar rows", () => {
  ...
  assert.match(
    templateSource,
    /export function ConfigSidebarItem[\s\S]*?className=\{\["config-sidebar-item"/,
  );
  assert.match(
    cssSource,
    /\.config-sidebar-item:not\(:disabled\):hover,[\s\S]*?background: var\(--bg-hover\)/,
  );
  assert.match(
    cssSource,
    /\.config-sidebar-item:focus-visible \{[\s\S]*?outline: 2px solid var\(--accent\)/,
  );
  assert.doesNotMatch(templateSource, /setHovered|setFocusVisible|useState/);
  assert.doesNotMatch(
    sources.SkillsConfig,
    /onMouseEnter[\s\S]*?var\(--bg-hover\)/,
  );
});
...
test("all shared config sidebar items use a fixed 30px height", () => {
  assert.match(
    cssSource,
    /\.config-sidebar-item \{[\s\S]*?height: 30px[\s\S]*?padding: 0 8px/,
  );
  assert.match(
    cssSource,
    /\.config-list-action-button \{[\s\S]*?height: 30px[\s\S]*?min-height: 30px/,
  );
});
...
test("skill scope group labels are localized", () => {
  ...
  for (const scope of ["global", "project", "path"]) {
    assert.match(
      skillsSource,
      new RegExp(`t\\("skills\\.scope\\.${scope}"\\)`),
    );
...
test("all subpanel detail panes share one content hierarchy", () => {
  const sources = Object.fromEntries(configSources);
  // 只截取 .config-detail-stack 规则块本身断言；[\s\S]*? 会跨规则块边界，
  // 误匹配文件后文（如 .settings-subagents-form）的 max-width: 720px。
  const detailStackBlock =
    cssSource.match(/\.config-detail-stack \{[^}]*\}/)?.[0] ?? "";
  assert.match(detailStackBlock, /gap: 16px/);
  assert.match(detailStackBlock, /width: 100%/);
  assert.doesNotMatch(detailStackBlock, /max-width: 720px/);
...
test("detail header actions keep buttons and switches aligned to the right", () => {
  ...
  assert.match(
    ...
    /<ConfigDetailActions>[\s\S]*?<ConfigSwitch[\s\S]*?<\/ConfigDetailActions>/,
  );
});
...
test("subpanel footers share sizing while maintenance actions stay secondary", () => {
  ...
  assert.match(
    cssSource,
    /\.config-footer-actions \{[\s\S]*?justify-content: flex-end/,
  );
  assert.match(
    cssSource,
    /\.config-footer-actions \.config-button-default \{[\s\S]*?min-width: 96px/,
  );
