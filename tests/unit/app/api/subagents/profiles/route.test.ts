...
test("profiles route creates, lists, and deletes a project profile", async () => {
  ...
  const source = await readFile(
    join(cwd, ".pi", "agents", "api-test-agent.md"),
    "utf8",
  );
  ...
  const getResponse = await GET(
    new Request(
      `http://localhost/api/subagents/profiles?cwd=${encodeURIComponent(cwd)}`,
    ),
  );
  const getBody = await getResponse.json();
  assert.equal(getResponse.status, 200);
  const listedProfile = getBody.profiles.find(
    (item) => item.name === "api-test-agent",
  );
  ...
  assert.equal(
    afterDeleteBody.profiles.some((item) => item.name === "api-test-agent"),
    false,
  );
});
...
test("profiles route keeps same-name global and project profiles independently editable", async () => {
  ...
  assert.match(
    await readFile(join(testAgentDir, "agents", "api-test-agent.md"), "utf8"),
    /Global profile/,
  );
  ...
  assert.deepEqual(
    sources.map((item) => item.description),
    ["Global profile", "Project profile"],
  );
  ...
  const toggledSources = (await response.json()).profiles.filter(
    (item) => item.name === "api-test-agent",
  );
  assert.equal(
    toggledSources.find((item) => item.scope === "global").enabled,
    false,
  );
  assert.equal(
    toggledSources.find((item) => item.scope === "global").description,
    "Global profile",
  );
  assert.equal(
    toggledSources.find((item) => item.scope === "global").loadSkills,
    true,
  );
  assert.equal(
    toggledSources.find((item) => item.scope === "global").loadExtensions,
    true,
  );
  assert.equal(
    toggledSources.find((item) => item.scope === "project").enabled,
    true,
  );
  ...
  response = await DELETE(
    jsonRequest("DELETE", { cwd, scope: "global", name: "api-test-agent" }),
  );
  assert.equal(response.status, 200);
});
...
test("profiles route rejects missing paths, malformed profiles, and unsafe names", async () => {
  ...
  response = await PUT(
    jsonRequest("PUT", {
      cwd,
      scope: "project",
      profile: profile({ thinking: "extreme" }),
    }),
  );
  assert.equal(response.status, 400);
  ...
  response = await PUT(
    jsonRequest("PUT", { cwd, scope: "workspace", profile: profile() }),
  );
  assert.equal(response.status, 400);
  ...
  response = await DELETE(
    jsonRequest("DELETE", { cwd, scope: "builtin", name: "Explore" }),
  );
  assert.equal(response.status, 400);
  assert.deepEqual(await response.json(), {
    error: "scope must be global or project",
  ...
  response = await PATCH(
    jsonRequest("PATCH", { cwd, scope: "project", name: "api-test-agent" }),
  );
  assert.equal(response.status, 400);
