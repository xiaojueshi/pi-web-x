...
function AgentRow({
  ...
  const relation =
    session.relation?.kind === "subagent" ? session.relation : null;
  const status: SubagentSessionStatus = running
    ? "running"
    : (relation?.status ?? "completed");
  const primary = main
    ? t("agentSwitcher.main")
    : relation?.description || sessionTitle(session);
    ...
    >
      <span
        style={{
          ...
          display: "grid",
          placeItems: "center",
          color: main ? "var(--text-muted)" : "var(--accent)",
        }}
      >
        {main ? (
          <svg
      ...
      <span
        style={{
          ...
          alignItems: "center",
          gap: 6,
          color: main && !running ? "var(--text-dim)" : statusColor(status),
          fontSize: 11,
          whiteSpace: "nowrap",
        }}
      >
...
export function AgentSessionPanel({
  ...
  const visibleSubagents = normalizedQuery
    ...
    : sortedSubagents;
  const runningCount = subagents.filter((session) =>
