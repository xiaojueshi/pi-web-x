...
async function getUploadDirectory(
  segments: string[],
): Promise<{ directory: string } | { response: HttpResponse }> {
...
export async function POST(
  ...
  try {
    ...
    if (type === "upload-check") {
      ...
      if (validationError) {
        return HttpResponse.json({ error: validationError }, { status: 400 });
      }
      return HttpResponse.json(inspectUploadTargets(directory, fileNames));
    }
    ...
    const strategy = parseUploadConflictStrategy(
      request.nextUrl.searchParams.get("conflict"),
    );
    if (!strategy) {
      return HttpResponse.json(
...
function streamFile(
  ...
  // a transcript) would otherwise run script in the Pi Web X origin, where it
...
export async function GET(
  ...
  try {
    ...
    return HttpResponse.json({ entries, path: filePath });
  } catch (error) {
