...
export async function POST(req: Request) {
  try {
    ...
    const baseUrl =
      typeof body.provider.baseUrl === "string"
        ? body.provider.baseUrl.trim()
        : "";
    if (!baseUrl)
      return HttpResponse.json(
    ...
    if (
      typeof body.provider.apiKey === "string" &&
      body.provider.apiKey.trim() &&
      !auth.apiKey
    ) {
      return HttpResponse.json(
    ...
    try {
      ...
      return HttpResponse.json(
        { error: "Upstream model list was not valid JSON" },
        { status: 502 },
      );
    }
    ...
    if (models.length === 0) {
      return HttpResponse.json(
        { error: "No models found in the upstream response" },
        { status: 502 },
      );
    }

    return HttpResponse.json({ models, endpoint: endpoint.toString() });
  } catch (error) {
