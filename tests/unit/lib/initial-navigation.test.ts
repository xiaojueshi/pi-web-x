...
async function loadSubject() {
  return import("../../../lib/initial-navigation.ts");
}
...
test("preserves a URL-encoded Windows path", async () => {
  ...
  assert.deepEqual(
    getInitialNavigation(new URLSearchParams("cwd=C%3A%5CProjects%5Cpi-web-x")),
    { requestedCwd: "C:\\Projects\\pi-web-x", sessionId: null },
  );
});
