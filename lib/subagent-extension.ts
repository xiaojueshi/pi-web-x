...
function agentTypeDescription(profiles: readonly SubagentProfile[]): string {
  ...
  if (available.length === 0)
    return "No subagent profiles are currently enabled.";
  return available
    .map((profile) => {
      const details = [
        `Tools: ${profile.tools.length > 0 ? profile.tools.join(", ") : "none"}`,
      ];
...
export function subagentFinalText(run: SubagentRunInfo): string {
  ...
  if (run.status === "completed")
    return run.result?.trim() || "Subagent completed without text output.";
  if (run.status === "aborted") return `Subagent ${run.sessionId} was stopped.`;
  if (run.status === "interrupted")
    return `Subagent ${run.sessionId} was interrupted before completion.`;
    ...
    factory: (pi) => {
      ...
      pi.registerTool(
        defineTool({
          ...
          promptSnippet:
            "Delegate a focused task to an inspectable subagent session",
          promptGuidelines: [
          ...
          parameters: Type.Object({
            subagent_type: Type.Optional(
              Type.String({
                description: `Configured agent profile. Available types: ${availableTypes}. Default: general-purpose.`,
              }),
            ),
            prompt: Type.String({
            ...
            input_files: Type.Optional(
              Type.Array(Type.String(), {
                description:
                  "UTF-8 text files under the session cwd to include with the task.",
                maxItems: MAX_SUBAGENT_INPUT_FILES,
              }),
            ),
            description: Type.String({
              description: "Short activity label shown in the UI.",
            }),
            run_in_background: Type.Optional(
              Type.Boolean({
                description:
                  "Return immediately and notify this session when complete.",
              }),
            ),
            model: Type.Optional(
              Type.String({
                description: "Optional provider/modelId override.",
              }),
            ),
            thinking: Type.Optional(
              Type.String({ description: "Optional thinking level override." }),
            ),
            max_turns: Type.Optional(
              Type.Number({
                description: "Optional positive agent turn limit.",
              }),
            ),
            inherit_context: Type.Optional(
              Type.Boolean({
                description:
                  "Include the parent session's active conversation context.",
              }),
            ),
          }),
          async execute(toolCallId, params, signal, onUpdate, ctx) {
            try {
              const execution = await runtime.start({
                ...
                ...(params.inherit_context !== undefined
                ...
                onUpdate: (run) =>
                  onUpdate?.({
                    content: [
                      {
                        type: "text",
                        text: `${run.profile}: ${run.description} (${run.status})`,
                      },
                    ],
              ...
              if (execution.run.runInBackground) {
                ...
                return {
                  content: [
                    {
                      type: "text",
                      text: `Subagent started in background. Session ID: ${execution.run.sessionId}. You will be notified when it completes.`,
                    },
                  ],
      ...
      pi.registerTool(
        defineTool({
          ...
          description:
            "Check an inspectable subagent session and retrieve its latest result.",
          parameters: Type.Object({
            agent_id: Type.String({ description: "Subagent session ID." }),
            wait: Type.Optional(
              Type.Boolean({
                description: "Wait until the subagent finishes.",
              }),
            ),
          }),
            ...
            if (!run)
              return {
                content: [
                  {
                    type: "text",
                    text: `Subagent not found: ${params.agent_id}`,
                  },
                ],
                details: undefined,
                isError: true,
      ...
      pi.registerTool(
        defineTool({
          ...
          label: "Steer agent",
          description:
            "Send a steering message to a currently running subagent session.",
          parameters: Type.Object({
            agent_id: Type.String({ description: "Subagent session ID." }),
            message: Type.String({
              description:
                "Instruction to inject after the current tool execution.",
            }),
          }),
...
/** Keep Pi Web X's integrated implementation when the legacy package is loaded. */
export function preferPiWebSubagentExtension(
  base: LoadExtensionsResult,
): LoadExtensionsResult {
  const host = base.extensions.find(
    (extension) => extension.path === HOST_SUBAGENT_EXTENSION_PATH,
  );
  ...
  const legacyPaths = new Set(
    base.extensions
      ...
      .filter((extension) => {
        ...
        return (
          sourcePackage === LEGACY_SUBAGENT_PACKAGE_NAME ||
          pathSegments.some(
            (segment) => segment === LEGACY_SUBAGENT_PACKAGE_NAME,
          )
        );
      })
      .filter((extension) =>
        [...SUBAGENT_TOOL_NAMES].some((name) => extension.tools.has(name)),
      )
  ...
  return {
    ...
    extensions: base.extensions.filter(
      (extension) => !legacyPaths.has(extension.path),
    ),
