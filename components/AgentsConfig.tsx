...
function displayProfilePath(
  ...
  if (
    ...
    profile.filePath.startsWith(cwd)
  ) {
...
export function AgentsConfig({
    ...
    async (preferredKey?: string) => {
      ...
      try {
        const response = await fetch(
          `/api/subagents/profiles?cwd=${encodeURIComponent(cwd)}`,
          { cache: "no-store" },
        );
        const data =
          (await response.json()) as Partial<SubagentProfilesResponse> & {
            error?: string;
          };
        ...
        setProfiles(next);
        const rememberedKey =
          preferredKey ?? getLastSettingsSelection("agents", cwd);
        const chosen =
          next.find((profile) => profileKey(profile) === rememberedKey) ??
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const data =
          (await response.json()) as Partial<SubagentSettingsResponse> & {
            error?: string;
          };
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch(
          `/api/models?cwd=${encodeURIComponent(cwd)}`,
          { signal: controller.signal },
        );
  ...
  const remove = async () => {
    ...
    if (
      !window.confirm(t("agents.deleteConfirm", { name: selected.displayName }))
    )
      return;
    setSaving(true);
    setError(null);
  ...
  const displayedPath = creating
    ...
    : selected
      ...
      : "";
  const fullPath = creating
    ? displayedPath
    : (selected?.filePath ?? displayedPath);
  const selectedModelAvailable =
    !draft.model ||
    modelOptions.some(
      (model) => `${model.provider}/${model.id}` === draft.model,
    );
  const selectedModel = (() => {
    ...
    return separator < 0
      ...
      : {
          ...
          modelId: draft.model.slice(separator + 1),
        };
  })();
  const controlStyle = disabled
    ? { ...inputStyle, ...disabledInputStyle }
    : inputStyle;
  const update = <K extends keyof EditableProfile>(
    key: K,
  ...
  const toggleEnabled = async (enabled: boolean) => {
    ...
    try {
      ...
      setProfiles((current) =>
        current.map((profile) =>
          profileKey(profile) === profileKey(saved) ? saved : profile,
        ),
      );
    ...
    >
      <div className="agents-feature-setting">
        <div className="agents-feature-copy">
          ...
          {reloadNeeded && (
            <span role="status" className="agents-feature-reload-notice">
      ...
      <ConfigSplitView>
        <ConfigSidebar>
          <ConfigSidebarList>
            {loading ? (
              <div
                style={{ padding: 10, color: "var(--text-dim)", fontSize: 12 }}
              >
                {t("agents.loading")}
              </div>
            ) : (
              (["project", "global", "workspace", "builtin"] as const).map(
                (scope) => {
                  ...
                  return (
                    <div key={scope} className="config-sidebar-group">
                      ...
                      {scopedProfiles.map((profile) => {
                        const overridden = isSubagentProfileOverridden(
                          profile,
                          profiles,
                        );
                        return (
                          <ConfigSidebarItem
                            ...
                            active={
                              selectedKey === profileKey(profile) && !creating
                            }
                            onClick={() => selectProfile(profile)}
                          >
                            <ConfigStatusDot active={profile.enabled} />
                            <ConfigSidebarText
                              className={`is-grow${profile.enabled ? "" : " is-muted"}`}
                            >
                              {profile.displayName}
                            </ConfigSidebarText>
                            {overridden && (
              ...
              <ConfigDetailStack>
                <ConfigDetailHeader>
                  <ConfigDetailHeaderInfo>
                    {displayedScope && (
                      <span
                        className={`config-scope-tag${displayedScope === "project" ? " is-project" : ""}`}
                      >
                  ...
                  <ConfigDetailActions>
                    ...
                    {selected &&
                      isWritableScope(selected.scope) &&
                      mode === "edit" && (
                        <ConfigButton
                          variant="danger"
                          size="small"
                    ...
                    <ConfigSwitch
                      ...
                      disabled={disabled}
                      label={
                        draft.enabled ? t("agents.disable") : t("agents.enable")
                      }
                ...
                >
                  ...
                  <Field label={t("agents.displayName")}>
                    <input
                      aria-label={t("agents.displayName")}
                ...
                <Field label={t("agents.resources")}>
                  ...
                  >
                    <Toggle
                      label={t("agents.loadSkills")}
                      disabled={disabled}
                      checked={draft.loadSkills}
                      onChange={(checked) => update("loadSkills", checked)}
                    />
                    <Toggle
                      label={t("agents.loadExtensions")}
                      disabled={disabled}
                      checked={draft.loadExtensions}
                      onChange={(checked) => update("loadExtensions", checked)}
                    />
                  </div>
                </Field>
                ...
                <div
                  style={{
                    ...
                    gridTemplateColumns: isMobile
                      ? "1fr"
                      : "minmax(0, 1.5fr) minmax(120px, 0.75fr) minmax(100px, 0.5fr)",
                    gap: 12,
                  }}
                >
                    ...
                    >
                      <ModelSelector
                        ...
                        disabled={
                          disabled ||
                          modelsLoading ||
                          (modelOptions.length === 0 && !draft.model)
                        }
                ...
                >
                  <Toggle
                    label={t("agents.inheritContext")}
                    disabled={disabled}
                    checked={draft.inheritContext}
                    onChange={(checked) => update("inheritContext", checked)}
                  />
                  <Toggle
                    label={t("agents.background")}
                    disabled={disabled}
                    checked={draft.runInBackground}
                    onChange={(checked) => update("runInBackground", checked)}
                  />
                </div>
              </ConfigDetailStack>
            )}
