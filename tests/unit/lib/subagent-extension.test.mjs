...
test("integrated extension exposes the legacy-compatible tool names", async () => {
  ...
  assert.equal(result.details.kind, "pi-web-x-subagent");
...
test("Agent tool description lists enabled effective profiles and refreshes when its factory reloads", async () => {
  ...
  assert.match(
    first.description,
    /- explore: Inspect the codebase \(Tools: read, grep\)/,
  );
  assert.doesNotMatch(first.description, /disabled-agent|Must stay hidden/);
  assert.match(
    first.parameters.properties.subagent_type.description,
    /Available types: explore/,
  );
  ...
  assert.match(
    ...
    /- reviewer: Review a change \(Tools: none; Model: provider\/review-model\)/,
  );
  assert.doesNotMatch(reloaded.description, /explore/);
  assert.match(
    reloaded.parameters.properties.subagent_type.description,
    /Available types: reviewer/,
  );
});

test("integrated extension removes a legacy extension that owns the same tools", () => {
  const host = {
    path: "<inline:pi-web-x-subagents>",
    tools: new Map([
      ["Agent", {}],
      ["get_subagent_result", {}],
      ["steer_subagent", {}],
  ...
  const result = preferPiWebSubagentExtension({
    ...
    errors: [
      { path: legacy.path, error: "duplicate" },
      { path: other.path, error: "other" },
    ],
  });

  assert.deepEqual(
    result.extensions.map((extension) => extension.path),
    [lookalike.path, host.path, other.path],
  );
  assert.deepEqual(
...
test("foreground Agent reports startup and execution failures as tool errors", async () => {
  ...
  const startup = await startFailureTools
    .get("Agent")
    .execute(
      "tool-call",
  ...
  const { tools: runFailureTools } = await loadTools({
    async start() {
      return {
        run: run(),
        completion: Promise.resolve(
          run({ status: "failed", error: "model failed" }),
        ),
      };
    },
    async get() {
  ...
  const failure = await runFailureTools
    .get("Agent")
    .execute(
      "tool-call",
      { prompt: "task", description: "desc" },
...
test("get_subagent_result wait honors abort signals", async () => {
  ...
  await assert.rejects(
    tools
      ...
      .execute(
        "call",
        { agent_id: "child-session", wait: true },
        controller.signal,
      ),
    /Result wait aborted/,
  );
...
test("steer_subagent returns success and runtime failures", async () => {
  ...
  const success = await tool.execute("call", {
  ...
  const failure = await tool.execute("call", {
    agent_id: "child-session",
    message: "fail",
  });
  assert.equal(failure.isError, true);
...
test("legacy preference leaves unrelated and partial-overlap extensions intact", () => {
  const host = { path: "<inline:pi-web-x-subagents>", tools: new Map() };
