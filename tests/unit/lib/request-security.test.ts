...
async function loadSubject() {
  return import("../../../lib/request-security.ts");
}
...
test("allows LAN same-origin requests for an internal localhost origin", async () => {
  const { isApiRequestAllowed } = await loadSubject();
...
test("allows IPv6 and an explicitly configured hostname", async () => {
  ...
  const configured = new Request("http://localhost:30141/api/test", {
    method: "POST",
    headers: {
      host: "pi-web-x.internal:30141",
      origin: "http://pi-web-x.internal:30141",
      "sec-fetch-site": "same-origin",
    },
  });
  assert.equal(isApiRequestAllowed(ipv6), true);
  assert.equal(isApiRequestAllowed(configured, ["pi-web-x.internal"]), true);
});
...
test("rejects missing, malformed, and unconfigured Host headers", async () => {
  ...
  assert.equal(
    isApiRequestAllowed(
      new Request("http://localhost:30141/api/test", {
        headers: { host: "pi-web-x.internal:30141" },
      }),
    ),
    false,
  );
