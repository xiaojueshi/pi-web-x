# Pi Web → Pi Web X：Bun 独立兼容实现迁移历史

> 状态：**已完成的历史记录**。本文保留最初的迁移目标、门禁与取舍，不是当前待办，也不是用户升级指南。当前架构见[架构概览](../development/architecture.md)，当前依赖状态见[运行时替代矩阵](../maintainers/runtime-substitution-matrix.md)，版本结果见 [`CHANGELOG.md`](../../CHANGELOG.md)。

[文档索引](../README.md) · [Bun/Node 边界](../development/bun-and-node.md)
...
唯一强制移除项是 **Next.js 及其专属构建链**。其余依赖不是“越少越好”的目标；只在功能等价、测试充分且收益明确时才替换。当前状态与未来复查条件见[运行时替代矩阵](../maintainers/runtime-substitution-matrix.md)。
...
| V6 | 测试基线 | 测试链迁移到 Bun 原生运行器；Playwright 在编译二进制上做 HTTP/PWA 黑盒验证 |
...
- 主测试链已迁移到 Bun 原生运行器：统一执行 `bun test`，不再依赖 Node test runner 或 jiti 启动链。
...
- [x] 在独立仓库中完成迁移，并保留 MIT LICENSE、版权与上游来源说明。
- [x] 锁定 upstream `0.8.11/28bab3c` 与 Bun 1.4.0 基线。
- [x] 完成 P0–P5 的可行性、Agent、配置、PWA 和发布门禁。
- [x] 建立并持续维护[运行时替代矩阵](../maintainers/runtime-substitution-matrix.md)。
- [x] 建立八平台构建、测试、Release notes 与 checksum 发布流程。

后续工作不再追加到本历史文档；当前改动应更新对应开发指南、维护者记录或 `CHANGELOG.md`。
