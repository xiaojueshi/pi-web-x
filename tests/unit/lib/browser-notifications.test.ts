...
async function loadSubject() {
  return import("../../../lib/browser-notifications.ts");
}
...
test("claims only blocking extension requests and deduplicates their ids", async () => {
  ...
  assert.equal(
    claimExtensionAttentionNotification(confirmRequest, notifiedIds),
    true,
  );
  assert.equal(
    claimExtensionAttentionNotification(confirmRequest, notifiedIds),
  ...
  for (const request of [
    ...
    assert.equal(
      claimExtensionAttentionNotification(request, notifiedIds),
      true,
    );
  }
  assert.equal(
