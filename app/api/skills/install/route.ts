...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!pkg?.trim())
      return HttpResponse.json({ error: "package required" }, { status: 400 });

    const isGlobal = scope !== "project";
    if (!isGlobal) {
      if (!cwd)
        return HttpResponse.json(
          { error: "cwd required for project install" },
          { status: 400 },
        );
    ...
    if (!success) {
      return HttpResponse.json(
        { error: output.slice(-300) || "Install failed" },
        { status: 500 },
      );
    }
    return HttpResponse.json({ success: true, output });
  } catch (e: unknown) {
    ...
    return HttpResponse.json(
      { error: output || (err.message ?? String(e)) },
      { status: 500 },
    );
  }
}
