...
test("defers only base64 images from historical tool results", () => {
  ...
  assert.match(
    deferred.messages[2].content[4].text,
    /1 tool result image omitted.*image\/tiff.*~1 bytes/,
  );

  const boundedFallback = buildSessionContext(entries, undefined, {
    deferToolResultImages: true,
  });
  assert.deepEqual(boundedFallback.messages[2].content[1], toolUrlImage);
  assert.match(
    boundedFallback.messages[2].content[2].text,
    /3 tool result images omitted.*image\/jpeg, image\/png, image\/tiff.*~9 bytes/,
  );
...
test("reads only a bounded session header, including headers larger than 4 KiB", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-x-header-"));
...
test("returns null for malformed or unbounded session headers", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-x-header-invalid-"));
...
test("session listing reads subagent relations and terminal status without reopening full session files", async () => {
  ...
  writeFileSync(
    ...
    [
      ...
      JSON.stringify({
        type: "custom",
        customType: "pi-web-x:subagent-result",
        id: "result",
        parentId: "meta",
        timestamp: "2026-01-01T00:00:01.000Z",
        data: {
...
test("resolves a matching session header without a catalogue scan", async () => {
  ...
  assert.equal(
    globalThis.__piPathToSessionIdCache?.get(sessionPathKey(filePath)),
    sessionId,
  );
