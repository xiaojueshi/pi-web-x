...
test("supports --help and -h without starting the server", () => {
  ...
  assert.match(getHelpText(), /Usage: pi-web-x/);
...
test("CLI writes help and parse errors before exiting", () => {
  ...
  assert.match(help.stdout, /Usage: pi-web-x/);
...
test("supports truthy PI_WEB_X_NO_OPEN values", () => {
  for (const value of ["1", "true", "TRUE", "yes", "on"]) {
    assert.equal(
      parseLaunchOptions([], { PI_WEB_X_NO_OPEN: value }).openBrowser,
      false,
    );
  }
});

test("does not disable browser opening for false PI_WEB_X_NO_OPEN values", () => {
...
test("supports PI_WEB_X_HOSTNAME without trusting the ambient system HOSTNAME", () => {
  assert.equal(
    parseLaunchOptions([], { HOSTNAME: "container-id" }).hostname,
    "127.0.0.1",
  );
  assert.equal(
    parseLaunchOptions([], { PI_WEB_X_HOSTNAME: "0.0.0.0" }).hostname,
    "0.0.0.0",
  );
});
