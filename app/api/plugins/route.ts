...
export async function GET(req: Request) {
  ...
  try {
    ...
    return HttpResponse.json(await readPlugins(cwd));
  } catch (error) {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!body.cwd)
      return HttpResponse.json({ error: "cwd required" }, { status: 400 });
    if (!body.action)
      return HttpResponse.json({ error: "action required" }, { status: 400 });
    const allowedRoots = await getAllowedFileRoots();
    ...
    return HttpResponse.json(await readPlugins(body.cwd));
  } catch (error) {
    return HttpResponse.json(
