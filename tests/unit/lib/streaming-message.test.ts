...
test("a reconnect snapshot replaces the old partial before deltas continue", () => {
  let state = snapshot(
    ...
    assistant([{ type: "text", text: "stale" }]),
  );
...
test("shows and streams a tool call after thinking, then accepts the authoritative end", () => {
  let state = snapshot(
    INITIAL_STREAMING_STATE,
    assistant([{ type: "thinking", thinking: "I will write the file." }]),
  );
  ...
  assert.equal(
    state.streamingMessage.content[1].rawInput,
    '{"path":"/tmp/file"',
  );
  ...
  assert.equal(
    Object.hasOwn(state.streamingMessage.content[1], "rawInput"),
    false,
  );
});
