...
const UNREAD_SESSIONS_STORAGE_KEY = "pi-web-x:unread-session-ids";
const LAST_CUSTOM_CWD_STORAGE_KEY = "pi-web-x:last-custom-cwd";
 ...
 * segments visible: "…orkspace/pi-web-x". Shows as much of the path as fits
...
function PiWebTitle() {
  ...
  const target = showVersion ? `${APP_VERSION}p${PI_VERSION}` : "Pi Web X";
...
export function SessionSidebar({
  ...
  const currentSuppressedCompletionSessionIdsRef = useRef<Set<string>>(
    new Set(),
  );
  const previousSuppressedCompletionSessionIdsRef = useRef<Set<string>>(
    new Set(),
  );
  ...
  const hasOtherWorkspaceActivity = useMemo(
    () =>
      [...projectActivity.entries()].some(
        ([key, { running, unread }]) =>
          key !== selectedProject?.key && (running > 0 || unread > 0),
      ),
                    ...
                    text={
                      currentWorktree
                        ? (currentWorktree.branch ??
                          displayCwd(currentWorktree.path, homeDir))
                        : "…"
                    }
                    style={{
                      flex: 1,
...
function SessionItem({
  ...
  const title =
    session.name || displayFirstMessage.slice(0, 50) || session.id.slice(0, 12);
