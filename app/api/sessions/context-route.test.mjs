...
const routeSrc = await readFileSync(
  new URL("./[id]/context/route.ts", import.meta.url),
  "utf8",
);
...
test("context route parses ?tail and ?before, excluding the boundary on paging", () => {
  ...
  assert.match(
    routeSrc,
    /buildSessionContext\(sm\.getEntries\(\) as never, before \?\? leafId, \{[^}]*excludeLeaf: Boolean\(before\)/,
  );
});
...
test("context route: ?before pages upward without duplicating the boundary", () => {
  ...
  assert.ok(
    ...
    "boundary `before` must not be duplicated",
  );
  assert.ok(
    page1.every((id) => !page2.includes(id)),
    "adjacent pages share no entry",
  );
});
