# Git worktrees in Pi Web X

[Documentation index](../README.md) · [简体中文](./worktrees.zh-CN.md)

Pi Web X can show all Git worktrees for one project in the sidebar. Use this when you want to keep separate checkouts for different branches, while keeping the project's sessions grouped together.
...
Use the worktree switcher to choose which checkout Pi Web X should use for new work in that project.
...
Pi Web X creates the checkout at:
...
```text
/Users/alex/Documents/Workspace/pi-web-x
```

and you create branch `codex/worktree-help`, the worktree is created under:

```text
/Users/alex/Documents/Workspace/pi-web-x-worktrees/codex-worktree-help
```

If the branch already exists, Pi Web X adds a worktree for that branch. If it does not exist, Pi Web X creates the branch from the current `HEAD`.
...
- The Git branch.
- Pi Web X session history.
- The main checkout.

If the worktree has uncommitted or untracked files, Git refuses the removal. Pi Web X then offers a force remove action. Force removal discards the uncommitted files in that checkout, so use it only when you no longer need those changes.

## Sessions and Worktrees

Pi Web X groups sessions by project root, so sessions from the main checkout and linked worktrees appear together.
...
Git can keep prunable worktree records after a checkout disappears. Pi Web X filters those out of the switcher.
