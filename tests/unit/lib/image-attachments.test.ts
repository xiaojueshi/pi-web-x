...
async function loadSubject() {
  return import("../../../lib/image-attachments.ts");
}
...
test("rejects invalid, oversized, and too many image attachments", async () => {
  const { MAX_ATTACHED_IMAGE_BYTES, MAX_ATTACHED_IMAGES, validateAgentImages } =
    await loadSubject();
  const oversizedData = "AAAA".repeat(
    Math.ceil((MAX_ATTACHED_IMAGE_BYTES + 1) / 3),
  );

  assert.equal(validateAgentImages([image]), null);
  assert.match(
    validateAgentImages([{ ...image, mimeType: "text/plain" }]),
    /valid base64 image/,
  );
  assert.match(
    validateAgentImages([{ ...image, data: oversizedData }]),
    /10MB/,
  );
  assert.match(
    validateAgentImages(
      Array.from({ length: MAX_ATTACHED_IMAGES + 1 }, () => image),
    ),
    /at most/,
  );
});
