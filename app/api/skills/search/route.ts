...
async function searchSkillsApi(
  ...
  return (data.skills ?? [])
    ...
    .sort(
      (a, b) => parseInstallCount(b.installs) - parseInstallCount(a.installs),
    );
}
...
export async function POST(req: Request) {
  try {
    ...
    if (results.length > 0) return HttpResponse.json({ results });
