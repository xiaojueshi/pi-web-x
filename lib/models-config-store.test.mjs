...
const { invalidateModelsCache, loadModelsWithCache } =
  await jiti.import("./models-cache.ts");
...
test("saving models.json atomically invalidates the model-list cache", async (t) => {
  ...
  const reloaded = await loadModelsWithCache(root, async () =>
    modelsData(`load-${++loads}`),
  );
...
test("models.json writes fill partial cost groups with zero and remove empty groups", (t) => {
    ...
    providers: {
      acme: {
        models: [
          ...
          {
            id: "complete-cost",
            cost: { input: 1, output: 2, cacheRead: 0.1, cacheWrite: 1.1 },
          },
        ],
        modelOverrides: {
  ...
  assert.deepEqual(config.providers.acme.models[0], {
    id: "empty-cost",
    cost: {},
  });
