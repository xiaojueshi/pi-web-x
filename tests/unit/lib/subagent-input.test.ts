...
test("loads cwd-relative UTF-8 files and formats them as delegated user input", async () => {
  ...
  try {
    ...
    assert.deepEqual(files, [
      { path: "docs/article.md", content: "# Article\n\nEvidence." },
    ]);
    assert.equal(
...
test("rejects files outside cwd, including symlink targets", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-x-subagent-input-cwd-"));
  const outside = await mkdtemp(
    join(tmpdir(), "pi-web-x-subagent-input-outside-"),
  );
  try {
    ...
    assert.throws(
      () => loadSubagentInputFiles(cwd, [outsideFile]),
      /outside the session cwd/,
    );
    assert.throws(
      () => loadSubagentInputFiles(cwd, ["linked.md"]),
      /outside the session cwd/,
    );
  } finally {
    await rm(cwd, { recursive: true, force: true });
...
test("rejects invalid UTF-8 and bounded input sets", async () => {
  ...
  try {
    ...
    await writeFile(
      join(cwd, "large.txt"),
      "x".repeat(MAX_SUBAGENT_INPUT_BYTES + 1),
      "utf8",
    );

    assert.throws(
      () => loadSubagentInputFiles(cwd, ["binary.txt"]),
      /not valid UTF-8/,
    );
    assert.throws(
      () => loadSubagentInputFiles(cwd, ["large.txt"]),
      /total limit/,
    );
    assert.throws(
      () =>
        loadSubagentInputFiles(
          cwd,
          Array(MAX_SUBAGENT_INPUT_FILES + 1).fill("binary.txt"),
        ),
      /at most/,
    );
  } finally {
