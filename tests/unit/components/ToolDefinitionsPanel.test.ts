...
const panelSource = await readFile(
  new URL("../../../components/ToolDefinitionsPanel.tsx", import.meta.url),
  "utf8",
);
const systemSource = await readFile(
  new URL("../../../components/SystemPromptPanel.tsx", import.meta.url),
  "utf8",
);
...
test("keeps System and Tools in separate adjacent toolbar actions", () => {
  assert.match(
    appShellSource,
    /handleSystemInfoToggle\("system", mobile\)[\s\S]*?handleSystemInfoToggle\("tools", mobile\)/,
  );
  assert.match(
    appShellSource,
    /activeTopPanel === "system"[\s\S]*?<SystemPromptPanel/,
  );
  assert.match(
    appShellSource,
    /activeTopPanel === "tools"[\s\S]*?<ToolDefinitionsPanel/,
  );
...
test("preserves the two-column layout on narrow screens", () => {
  ...
  assert.doesNotMatch(
    panelSource,
    /@media \(max-width: 640px\)[\s\S]*?\.tool-definitions-panel \{[\s\S]*?display: block/,
  );
});
