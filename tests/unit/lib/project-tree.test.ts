...
const { MAX_PROJECTED_TREE_DEPTH, projectTreeForResponse } = await import(
  "../../../lib/project-tree.ts"
);
...
test("attaches the first branch message preview to the contracted representative", () => {
  ...
  const arm2 = node(msg("u2b", "user", "分支二的问题"), [
    node(msg("a2b", "assistant", "答二")),
  ]);
  ...
  const assistant = node(
    msg("a1", "assistant", [
      ...
      { type: "text", text: "可见回答" },
      {
        ...
        id: "tc1",
        name: "read",
        arguments: { value: "tool-secret" },
      },
    ]),
    [node(info("leaf1"))],
  );
  const sibling = node(msg("u2", "user", "另一个分支"), [node(info("leaf2"))]);
  const projected = projectTreeForResponse([
    node(info("root"), [assistant, sibling]),
  ]);
...
test("carries previews through non-message roots in multi-root trees", () => {
  const root1 = node(model("m1"), [
    node(msg("u1", "user", "第一个问题"), [
      node(msg("a1", "assistant", "回答一")),
    ]),
  ]);
  const root2 = node(info("s2"), [
    node(msg("u2", "user", "第二个问题"), [
      node(msg("a2", "assistant", "回答二")),
    ]),
  ]);
  const projected = projectTreeForResponse([root1, root2]);

  assert.deepEqual(projected[0].children[0].branchPreview, {
    role: "user",
    text: "第一个问题",
  });
  assert.deepEqual(projected[1].children[0].branchPreview, {
    role: "user",
    text: "第二个问题",
  });
...
test("normalizes and bounds preview text and labels image-only messages", () => {
  ...
  const projected = projectTreeForResponse([
    node(info("root"), [textArm, imageArm]),
  ]);
...
test("carries previews through the depth-limit flattening path", () => {
  let deepArm = node(info("prefix"), [
    node(msg("deep-user", "user", "深层问题"), [node(info("deep-leaf"))]),
  ]);
...
test("linear sessions still project to root + leaf only", () => {
  const root = node(msg("u1", "user", "第一问"), [
