...
test("shows all render items when the visible count reaches the total", async () => {
  const { getVisibleRenderWindow } = await loadSubject();
  assert.deepEqual(getVisibleRenderWindow(30, 50), {
    startIndex: 0,
    hasMore: false,
  });
  assert.deepEqual(getVisibleRenderWindow(50, 50), {
    startIndex: 0,
    hasMore: false,
  });
  assert.deepEqual(getVisibleRenderWindow(0, 50), {
    startIndex: 0,
    hasMore: false,
  });
});
...
test("continues paging when render items outnumber source messages", async () => {
  ...
  assert.deepEqual(getVisibleRenderWindow(120, visibleCount), {
    startIndex: 20,
    hasMore: true,
  });
...
test("treats only the real message tail as live-follow attached", async () => {
  ...
  for (const lines of [1, 2, 3, 4]) {
    assert.equal(
      getLiveFollowAttached(false, 380, 400, 600, 1000 + lines * 24),
      true,
    );
  }
...
test("prompt anchor spacer includes the viewport deficit for short conversations", async () => {
  ...
  let spacerHeight = getPromptAnchorSpacerHeight(
    targetTop,
    contentHeight,
    clientHeight,
  );
  ...
  contentHeight += 24;
  spacerHeight = getPromptAnchorSpacerHeight(
    targetTop,
    contentHeight,
    clientHeight,
  );
