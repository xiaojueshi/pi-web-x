...
test("keeps project settings selections isolated by cwd", () => {
  ...
  assert.equal(
    getLastSettingsSelection("skills", "/project-one", storage),
    "/one/skill.md",
  );
  assert.equal(
    getLastSettingsSelection("skills", "/project-two", storage),
    "/two/skill.md",
  );
  assert.equal(
    getLastSettingsSelection("skills", "/project-three", storage),
    null,
  );
});
...
test("shares the models selection globally", () => {
  ...
  assert.equal(
    getLastSettingsSelection("models", "/project-two", storage),
    selection,
  );
...
test("ignores malformed and unavailable browser storage", () => {
  const malformed = createStorage({ "pi-web-x:settings-navigation": "{" });
  const unavailable = {
    getItem() {
  ...
  assert.equal(
    getLastSettingsSelection("plugins", "/project", malformed),
    null,
  );
  ...
  assert.doesNotThrow(() =>
    setLastSettingsSelection("plugins", "key", "/project", unavailable),
  );
});
