...
test("PowerShell is opt-in on Windows and never selected on other platforms", () => {
  ...
  assert.equal(
    isPowerShellToolEnabled(["read", "bash", "powershell"], "win32"),
    false,
  );
  assert.equal(
    isPowerShellToolEnabled(["read", "powershell"], "darwin"),
    false,
  );
  assert.deepEqual(
    resolveShellTools(
...
test("the setting preserves unrelated config and switches defaultTools both ways", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-web-x-powershell-"));
...
test("enabling PowerShell adds it when a custom defaultTools list has no shell", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-web-x-powershell-custom-"));
  const settingsPath = join(root, "settings.json");
  await writeFile(
    settingsPath,
...
test("invalid defaultTools are rejected without overwriting the settings file", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-web-x-powershell-invalid-"));
  const settingsPath = join(root, "settings.json");
