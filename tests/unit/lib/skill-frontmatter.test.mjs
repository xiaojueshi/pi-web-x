...
describe("setDisableModelInvocation", () => {
  ...
  it("adds the key after the opening fence when absent", () => {
    ...
    assert.equal(
      parseFrontmatter(updated).frontmatter["disable-model-invocation"],
      true,
    );
  });
  ...
  it("updates and removes indented quoted keys", () => {
    for (const quote of ['"', "'"]) {
      ...
      assert.equal(
        parseFrontmatter(setDisableModelInvocation(updated, false)).frontmatter[
  ...
  it("rejects unsupported key formatting instead of silently succeeding", () => {
    ...
    assert.throws(
      ...
      /unsupported frontmatter formatting/,
    );
  });
  ...
  it("preserves CRLF line endings when updating or adding the key", () => {
    ...
    assert.equal(
      setDisableModelInvocation(
        content.replace("disable-model-invocation: false\r\n", ""),
        true,
      ),
      "---\r\ndisable-model-invocation: true\r\nname: my-skill\r\n---\r\nBody text.\r\n",
    );
  ...
  it("is a no-op when disabling is off and the key is absent", () => {
    assert.equal(
      setDisableModelInvocation(withFrontmatter, false),
      withFrontmatter,
    );
  });
