...
export function getUpdatedExtensionWidgetKeys(
  ...
  return Array.from(next, ([key, lines]) => {
    ...
    return lines.some((line, index) => line !== previousLines[index])
      ? key
      : null;
...
export function ExtensionWidgets({
  ...
  useEffect(() => {
    ...
    setUpdatingWidgetKeys((current) => {
      const next = new Set(
        Array.from(current).filter((key) => nextContents.has(key)),
      );
  ...
  useEffect(
    () => () => {
      for (const timer of updateClearTimersRef.current.values())
        clearTimeout(timer);
      ...
      >
        {widgets.map((widget, index) => {
          ...
          const content = (
            <>
              <span
                className="extension-widget-update-pulse"
                aria-hidden="true"
              />
              <span className="extension-widget-placement" aria-hidden="true">
                <svg
                  ...
                  height="6"
                  data-direction={
                    widget.placement === "belowEditor" ? "down" : "up"
                  }
                  focusable="false"
                >
                  <path
