...
function paths(content, toolResults, cwd) {
  return extractTurnWrittenFiles(content, toolResults, cwd).map(
    (f) => f.filePath,
  );
}
...
test("resolves a relative path against cwd", () => {
  ...
  assert.deepEqual(paths(content, results(okResult("1")), "/abs"), [
    "/abs/out/report.html",
  ]);
});
...
test("preserves path characters that have special meaning in hrefs", () => {
  ...
  assert.deepEqual(
    paths(
      content,
      results(okResult("1"), okResult("2"), okResult("3")),
      "/repo",
    ),
...
test("normalizes Windows-relative tool paths against a Windows cwd", () => {
  ...
  assert.deepEqual(paths(content, results(okResult("1")), "C:\\repo"), [
...
test("ignores paths that only appear in the reply text", () => {
  ...
  const content = [
    {
      type: "text",
      text: "I saved the report to /abs/out/report.html for you.",
    },
  ];
  assert.deepEqual(paths(content, results()), []);
});
