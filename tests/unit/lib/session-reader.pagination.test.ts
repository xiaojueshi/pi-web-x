...
test("buildSessionContext excludeLeaf pages upward without duplicating `before`", () => {
  ...
  const page2 = buildSessionContext(entries, oldest, {
    tail: 5,
    excludeLeaf: true,
  }).entryIds;
  assert.equal(page2[page2.length - 1], "e47");
  assert.ok(
    !page2.includes(oldest),
    "page2 must not duplicate the `before` boundary",
  );
...
test("pagination stops before the root instead of returning it again", () => {
  ...
  const page = buildSessionContext(entries, "e0", {
    tail: 5,
    excludeLeaf: true,
  });
...
test("pagination cursor follows the raw page boundary across compaction", () => {
  ...
  const page2 = buildSessionContext(entries, page1.oldestEntryId, {
    tail: 3,
    excludeLeaf: true,
  });
...
test("real sessions may store assistant content as a string (deferThinking guard)", () => {
  ...
  const entries = [
    ...
    },
    {
      ...
      message: {
        role: "assistant",
        content: "a string reply, not a block array",
      },
    },
  ];
  ...
  assert.deepEqual(ctx.messages[1].content, [
    { type: "text", text: "a string reply, not a block array" },
  ]);
});
