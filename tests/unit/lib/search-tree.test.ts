...
async function loadSubject() {
  return import("../../../lib/search-tree.ts");
}
...
test("groups flat paths into directories with files at the root level", async () => {
  ...
  assert.deepEqual(names(components.children), [
    "FileExplorer.tsx",
    "SessionSidebar.tsx",
  ]);
...
test("builds arbitrarily deep nesting", async () => {
  ...
  assert.deepEqual(
    [api.name, files.name, bracketDir.name],
    ["api", "files", "[...path]"],
  );
  for (const dir of [app, api, files, bracketDir])
