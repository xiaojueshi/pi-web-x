...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  const subagentSource = await readFile(
    new URL("../../../lib/subagent-runtime.ts", import.meta.url),
    "utf8",
  );
  const startupSource = source.slice(
    source.indexOf("export async function startRpcSession"),
...
test("running snapshots expose sessions with suppressed completion notifications", async () => {
  ...
  assert.match(
    runningRouteSource,
    /completionNotificationSuppressedSessionIds:\s*getCompletionNotificationSuppressedRpcSessionIds\(\)/,
  );
  assert.match(
    sessionsRouteSource,
    /completionNotificationSuppressedSessionIds:\s*getCompletionNotificationSuppressedRpcSessionIds\(\)/,
...
test("RPC session startup treats only sessions with messages as continuing", async () => {
  ...
  assert.match(
    startupSource,
    /const hasExistingMessages = sessionManager\s*\.getBranch\(\)\s*\.some\([\s\S]*?entry\) => entry\.type === "message"\)/,
  );
...
test("session replacement rejects active work and clone writes one reopenable child", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-web-x-clone-"));
...
test("clone cancels an assistant-free branch without creating a file", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-web-x-clone-empty-"));
  const sessionDir = join(root, "sessions");
  await mkdir(sessionDir);
  const manager = SessionManager.create(root, sessionDir);
  manager.appendMessage({
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(
    startupSource,
    /const selectedToolNames\s*=\s*subagentResources\?\.tools\s*\?\?\s*persistedToolNames\s*\?\?\s*requestedToolNames/,
  );
  assert.match(
    startupSource,
  ...
  assert.match(
    startupSource,
    /chatOnly\s*\?\s*CHAT_ONLY_RESOURCE_LOADER_OPTIONS/,
  );
  assert.match(
    startupSource,
