...
function getSubagentStartingCounts(): Map<string, number> {
  if (!globalThis.__piSubagentStartingCounts)
    globalThis.__piSubagentStartingCounts = new Map();
...
function parseSubagentModel(runtime: ModelRuntime, value: string | undefined) {
  ...
  if (matches.length === 0)
    throw new Error(`Subagent model not found: ${requested}`);
  throw new Error(
    `Subagent model is ambiguous; use provider/modelId: ${requested}`,
  );
}
...
function reserveSubagentSlot(parentSessionId: string): () => void {
  ...
  if (active + startingCount >= MAX_CONCURRENT_SUBAGENTS) {
    throw new Error(
      `A session can run at most ${MAX_CONCURRENT_SUBAGENTS} subagents at once`,
    );
  }
...
export function createSubagentController(
  ...
  async function start(
    ...
    if (!enabled())
      throw new Error("Pi Web X built-in sub-agents are disabled");
    ...
    if (!parent.sessionFile)
      throw new Error(
        "Parent session must be persisted before starting a subagent",
      );
    ...
    try {
      ...
      const runInBackground =
        request.runInBackground ?? profile.runInBackground;
      ...
      const turnLimit =
        maxTurns && maxTurns > 0 ? Math.floor(maxTurns) : undefined;
      const thinking =
        request.thinking ??
        profile.thinking ??
      ...
      const inheritedParentContext = inheritContext
        ...
        : undefined;
      const inputFiles = loadSubagentInputFiles(
        parent.cwd,
        request.inputFiles ?? [],
      );
      ...
      const services = await createAgentSessionServices({
        ...
        ...(profile.loadExtensions || profile.loadSkills
          ? {
              resourceLoaderReloadOptions: projectTrustReloadOptions(
                parent.cwd,
                agentDir,
              ),
            }
      ...
      const sessionManager = SessionManager.create(parent.cwd, undefined, {
      ...
      const requestedModel = parseSubagentModel(
        parentModelRuntime,
        request.model ?? profile.model,
      );
      const parentModel = parent.inner.model as ReturnType<
      ...
      if (!runInBackground)
        request.signal?.addEventListener("abort", handleParentAbort, {
          once: true,
        });
  ...
  async function notifyParent(run: SubagentRunInfo): Promise<void> {
    ...
    if (!parent?.isAlive()) {
      const sessionFile = await dependencies.resolveSessionPath(
        run.parentSessionId,
      );
      if (!sessionFile)
        throw new Error(`Parent session not found: ${run.parentSessionId}`);
      parent = await dependencies.reopenSession(
        run.parentSessionId,
        sessionFile,
      );
    }
    ...
    if (!parent.isAlive())
      throw new Error(
        `Parent session is no longer available: ${run.parentSessionId}`,
      );
    await parent.inner.sendCustomMessage(
      {
        customType: "pi-web-x:subagent-notification",
