...
export function loadSubagentInputFiles(
  cwd: string,
  requestedPaths: readonly string[],
): SubagentInputFile[] {
  if (requestedPaths.length > MAX_SUBAGENT_INPUT_FILES) {
    throw new Error(
      `Agent input_files accepts at most ${MAX_SUBAGENT_INPUT_FILES} files`,
    );
  }
  ...
  for (const requestedPath of requestedPaths) {
    if (!requestedPath.trim())
      throw new Error("Agent input_files paths must not be empty");
    ...
    if (!isPathWithinRoots(filePath, allowedRoots)) {
      throw new Error(
        `Agent input file is outside the session cwd: ${requestedPath}`,
      );
    }
    ...
    if (totalBytes > MAX_SUBAGENT_INPUT_BYTES) {
      throw new Error(
        `Agent input_files exceeds the ${MAX_SUBAGENT_INPUT_BYTES}-byte total limit`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Agent input file is not valid UTF-8 text: ${requestedPath}`,
      );
    }
    files.push({
...
export function appendSubagentInputFiles(
  task: string,
  files: readonly SubagentInputFile[],
): string {
