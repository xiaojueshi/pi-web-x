...
import { isTodoToolDetails } from "@/lib/todo-details";
...
function isSubagentToolDetails(value: unknown): value is SubagentToolDetails {
  ...
  const details = value as { kind?: unknown; sessionId?: unknown };
  return (
    details.kind === "pi-web-x-subagent" &&
...
function ToolCallBlock({
  ...
  const todoDetails =
    !isError && isTodoToolDetails(result?.details) ? result.details : null;
  const subagent = isSubagentToolDetails(result?.details)
    ? result.details
    : null;
  ...
  return (
    ...
    >
      ...
      {/* ── Todo 进度卡片：常显，无需展开 ── */}
      {todoDetails && <TodoListBody details={todoDetails} />}
...
/**
 * todo 工具的进度卡片：常显在工具调用块头部下方，展示完整待办列表与
 * 完成进度（进度条 + 逐项勾选状态）。数据来自工具结果的 details 快照，
 * 历史消息（会话文件里的旧工具结果）同样可渲染。
 */
function TodoListBody({ details }: { details: TodoDetails }) {
  const { t } = useI18n();
