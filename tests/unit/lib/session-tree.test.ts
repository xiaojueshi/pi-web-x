...
const { buildSessionTree } = await createJiti(import.meta.url).import(
  "../../../lib/session-tree.ts",
);
...
test("only subagents nest while forks remain independent roots", () => {
  const roots = buildSessionTree([
    ...
    session("fork", "2026-01-03T00:00:00.000Z", {
      kind: "fork",
  ...
  assert.deepEqual(
    roots.map((node) => node.session.id),
    ["fork", "parent"],
  );
  assert.deepEqual(
...
test("orphaned subagents become roots and every level is sorted newest first", () => {
  const roots = buildSessionTree([
    session("parent", "2026-01-01T00:00:00.000Z"),
    session("older", "2026-01-02T00:00:00.000Z", {
      kind: "subagent",
      parentSessionId: "parent",
      profile: "A",
      description: "A",
    }),
    session("newer", "2026-01-03T00:00:00.000Z", {
      kind: "subagent",
      parentSessionId: "parent",
      profile: "B",
      description: "B",
    }),
    session("orphan", "2026-01-04T00:00:00.000Z", {
      kind: "subagent",
...
test("cyclic relation metadata cannot hide sessions or recurse forever", () => {
  ...
  assert.deepEqual(
    roots.map((node) => node.children.length),
    [0, 0],
  );
});
